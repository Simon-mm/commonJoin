			...
			<div class="wrap">
				<div class="top-link fr">					
					<c:if test="${empty login }">
						...
						<span>/</span><a href="register.jsp" title="注册">注册</a>
					</c:if>
					<c:if test="${not empty login }">
						<a href="login?status=${login.status }" target="_blank" title="">欢迎你，${login.id }用户</a>
						<span>/</span><a href="loginOut" title="注销">注销</a>
					</c:if>
				</div>
				<span><script language="javascript" src="${pageContext.request.contextPath}/js_other/news/mp4video.js"></script>
					<p>湖北汽车工业学院学生社团网站！</p>
				</span>
			</div>
		</div>
	...
	<div class="padding-40 cleafix">
		<div class="wrap cleafix">
			<div class="nrfl fl">
				<h3>活力社团</h3>
				<ul class="cleafix">
					<li class="hoverl"><a href="#" title="活力社团"
						class="hover">活力社团</a></li>
				</ul>
			</div>
			<div class="nrfr fr cleafix">
				...
				<script language="javascript"
					src="${pageContext.request.contextPath}/js_other/centerCutImg.js"></script>
				...
				<ul class="list cleafix">
					<c:if test="${not empty clublist }">
						<c:forEach items="${clublist }" var="clist">
						<li id="line_u10_0">
							<span class="fr gay"><fmt:formatDate value="${clist.createDate }" pattern='yyyy-MM-dd'/></span>
							<a href="getClubInfo?cid=${clist.id }" target="_blank" title="活力社团 | ${clist.clubName }">活力社团     | ${clist.clubName }</a>
						</li></c:forEach>
					</c:if>
				...
				<div class="pb_sys_common pb_sys_normal pb_sys_style2"
					...
					<span class="p_pages"><span class="p_first_d p_fun_d">首页</span><span
						class="p_prev_d p_fun_d">上页</span><span class="p_no_d">1</span><span
						class="p_next_d p_fun_d">下页</span><span class="p_last_d p_fun_d">尾页</span></span>
				</div>
