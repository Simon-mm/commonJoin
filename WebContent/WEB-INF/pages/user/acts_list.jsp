...
<script type="text/javascript">	
	$(function(){
		...
		var actStatus=[];//存放每个活动的进行状态
		var str = '${jsonStr }';
		var obj = JSON.parse(str);//将后端传来的JSON字符串转为JSON对象（是单引号套双引号）
		for(var i=0;i<obj.length;i++){
			//alert(obj[i].actStart);
			if( nowTime.getTime() < new Date(obj[i].actStart).getTime() ){
				actStatus[i] = "活动发布中";
				//alert('还未开始');
			}				
			else if( nowTime.getTime() > new Date(obj[i].actStart).getTime() && nowTime.getTime() < new Date(obj[i].actEnd).getTime() ){
				actStatus[i] = "活动进行中";
				//alert('进行中……');
			}				
			else{
	...
	<td align="left" valign="top">
		<table width="100%" border="0" cellspacing="0" cellpadding="0" id="main-tab">
			<tr>
		        ...
		        <th align="center" valign="middle">活动进度</th>
			...
			<tr onMouseOut="this.style.backgroundColor='#ffffff'" onMouseOver="this.style.backgroundColor='#edf5ff'">
		        ...
		        <td align="center" valign="middle" class="borderright borderbottom">${joinNum[status.index] } / ${act.actNum}</td>
				...
				<c:choose>
					...
					<c:when test="${act.scope eq 1 }">校级范围</c:when>
				</c:choose>
				</td>
				<!-- 活动进度 -->
				<td id="process" align="center" valign="middle" class="borderright borderbottom">
				<c:choose>
				...
				<td align="center" valign="middle" class="borderright borderbottom">
				<c:choose>
					<c:when test="${isJoin[status.index] eq 0  && actStatus[status.index] eq 0}">
					<!-- 先通过js事件判断是否登录，若没有则提示登录，否则可以进行报名活动操作！ -->
					<a href="toSignAct?uid=${login.id }&actid=${act.actID}" onclick="return isLogin();return true;" style="text-decoration: none;font-size: 15px">报名活动</a>
					</c:when>
					<c:when test="${isJoin[status.index] eq 1 }"><font color="green">已报名</font></c:when>
					<c:when test="${isJoin[status.index] eq 2 && actStatus[status.index] eq 2}"><font color="grey">活动已结束，不可报名</font></c:when>
					<c:when test="${isJoin[status.index] eq 2 }"><font color="red">不在报名范围内，不可报名</font></c:when>
					<c:when test="${isJoin[status.index] eq 3 }"><font color="red">已达报名人数限额，不可报名</font></c:when>
				</c:choose>				
				</td>
			</tr>
			</c:forEach>		
