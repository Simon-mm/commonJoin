  ...
  <tr>
    ...
    <form id="sign_act" method="post" action="" >
      ...
      <tr class="bggray" onMouseOut="this.style.backgroundColor='#f9f9f9'" onMouseOver="this.style.backgroundColor='#edf5ff'">
        ...
        <c:choose>
	        <c:when test="${formList.finished eq 0}"><font color="orange">待处理</font></c:when>
	        <c:when test="${formList.finished eq 1}"><font color="green">已录取</font></c:when>
	        <c:when test="${formList.finished eq 2}"><font color="red">已拒绝</font></c:when>
	        <c:otherwise>未知状态</c:otherwise>
	    </c:choose>
        </td>        
        <td align="center" valign="middle" class="borderbottom">        
        <c:choose>
        <c:when test="${formList.finished eq 0}">
		<a href="solveActSignApply?uid=${formList.uid }&aid=${formList.actid }&flag=p" style="text-decoration: none;color: green;" onclick="process();return false;" target="mainFrame" onFocus="this.blur()" class="add">通过</a>
		<span class="gray">&nbsp;|&nbsp;</span>
		<a href="solveActSignApply?uid=${formList.uid }&aid=${formList.actid }&flag=r" style="text-decoration: none;color: red;" onclick="process();return false;" target="mainFrame" onFocus="this.blur()" class="add">拒绝</a>
		</c:when>
		<c:when test="${formList.finished gt 0 }">
